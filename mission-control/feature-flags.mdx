...
1. **Features** are a global registry of everything that can be toggled (e.g., `high_cpa_alerts`, `creative_reporting`). Each feature has a unique key, a description, and a global active/inactive switch.
...
<Step title="Click Create Feature">
  ...
  - **Key** is a unique identifier used in code to check the flag (e.g., `high_cpa_alerts`). This is what developers reference in `isEnabled('high_cpa_alerts')`, so keep it lowercase with underscores.
...
Say you want to show high CPA alerts on the Executive Summary only for clients who have it enabled. Inject the service and expose a getter:
...
export class ExecutiveSummaryComponent {
  ...
  get showHighCpaAlerts(): boolean {
    return this.featureFlagService.isEnabled('high_cpa_alerts');
  }
}
```

Then conditionally render the section in the template:

```html
<!-- executive-summary.component.html -->
<section *ngIf="showHighCpaAlerts" class="cpa-alerts">
  <h3>High CPA Alerts</h3>
  <app-cpa-alert-table [data]="cpaAlerts"></app-cpa-alert-table>
</section>
```

### Gating an entire page

If you want to restrict access to an entire page like Creative Reporting, check the flag early and show a fallback:

```typescript
// creative-reporting.component.ts
export class CreativeReportingComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.hasAccess = this.featureFlagService.isEnabled('creative_reporting');
  }
}
```

```html
<!-- creative-reporting.component.html -->
<div *ngIf="hasAccess; else noAccess">
  <!-- full report content -->
</div>

<ng-template #noAccess>
  <p>Creative Reporting is not enabled for your account. Contact your admin.</p>
</ng-template>
```
...
// { "high_cpa_alerts": true, "creative_reporting": false, ... }
```
...
|-------|------|-------------|
| `key` | `CharField(100)` | Unique identifier (e.g., `high_cpa_alerts`) |
